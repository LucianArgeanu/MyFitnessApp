...
class ShakeTableViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    

    @IBOutlet weak var tableView: UITableView!
    var shakeContentMenuArray = [ShakeMenuContent]()
    
    override func viewDidLoad() {
        super.viewDidLoad()
        initTableView()
        GetShakeContent.getData { [weak self] (shakeContent) in
            self?.shakeContentMenuArray = shakeContent
            self?.tableView.reloadData()
        }
    }
    
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return shakeContentMenuArray.count
    }
    
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "CellTable") as! CellTable
        let cellInfo = shakeContentMenuArray[indexPath.row]
        cell.imageCell.sd_setImage(with: cellInfo
            .imageURL, completed: nil)
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        controller.setupShake(with: shakeContentMenuArray[indexPath.row])
    }
